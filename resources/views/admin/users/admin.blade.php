          ...
          <div class="relative bg-white dark:bg-gray-800 rounded-t-lg">
            ...
            <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
              <div class="w-full md:w-1/2">
                <form action="{{route('admin.admin.search')}}" method="GET" class="flex items-center">
              ...
              <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 rounded-lg">
                  ...
                  <tbody>
                      ...
                      <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                          <td class="w-4 p-4">
                              <div class="flex items-center">
                                <div class="text-base font-semibold">{{ $admins->firstItem() + $loop->index }}</div>
                              </div>
                          </td>
                          <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                            @if ($admin->image)
                              <img class="w-10 h-10 rounded-full" src="{{asset('profile/' . $admin->image)}}" alt="Admin Image">
                            @else
                              <img class="w-10 h-10 rounded-full" src="{{asset('profile/default.png')}}" alt="Admin Image">
                            @endif
                            <div class="pl-3">
                              <div class="text-base font-semibold">{{ ucfirst($admin->firstname) . " ". ucfirst(substr($admin->middlename, 0, 1)) . " " . ucfirst($admin->lastname) }}</div>
                          ...
                          <td class="px-6 py-4">
                              <div class="flex items-center space-x-4 justify-center">
                                <a href="{{route('admin.user.edit', ['user' => $admin->id])}}" class="py-2 px-3 flex items-center text-sm font-medium text-center text-white bg-teal-700 rounded-lg hover:bg-teal-800 focus:ring-4 focus:outline-none focus:ring-teal-300 dark:bg-teal-600 dark:hover:bg-teal-700 dark:focus:ring-teal-800">
                                ...
                                <button type="button" data-modal-target="admin-preview{{$admin->id}}" data-modal-toggle="admin-preview{{$admin->id}}" class="py-2 px-3 flex items-center text-sm font-medium text-center text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-teal-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                                ...
                                <button type="button" data-modal-target="delete-modal{{$admin->id}}" data-modal-toggle="delete-modal{{$admin->id}}" class="flex items-center text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-2 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 -ml-0.5" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
                      ...
                      @include('admin.users.includes.admin-includes')

                      @include('admin.users.includes.admin-delete')
            ...
            <nav class="flex flex-col items-start justify-between p-4 space-y-3 md:flex-row md:items-center md:space-y-0" aria-label="Table navigation">
                ...
                <ul class="inline-flex items-stretch -space-x-px">
                    ...
                    @foreach (range($startPage, $endPage) as $page)
                        <li>
                            <a href="{{ $admins->url($page) . (empty(request()->input('query')) ? '' : '&query=' . request()->input('query')) }}"
