@include('admin.partials.header')
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-t-lg">
                ...
                <div class="flex items-center p-4 text-base text-blue-800 bg-blue-50 dark:bg-blue-900 dark:text-blue-400" role="alert">
                    ...
                    <div class="text-justify">
                        ...
                        <strong>
                            {{ \Carbon\Carbon::parse($schedules->departure_date . ' ' . $schedules->departure_time)->format('M d, Y g:i a') }}
                        </strong>
                        to
                        <strong>
                            {{ \Carbon\Carbon::parse($schedules->arrival_date . ' ' . $schedules->arrival_time)->format('M d, Y g:i a') }}
                        </strong>.
                    </div>
                </div>
                <div class="flex items-center p-4 text-base text-blue-800 bg-blue-50 dark:bg-blue-900 dark:text-blue-400" role="alert">
                    ...
                    <span class="sr-only">Info</span>
                    <div class="text-justify">
                        Trip Information: <strong>{{$schedules->departure_port}}</strong> to <strong>{{$schedules->arrival_port}}</strong> using <strong>{{$schedules->ferries->name}}</strong>.
                    </div>
                </div>
                <div class="flex items-center p-4 text-base text-blue-800 bg-blue-50 dark:bg-blue-900 dark:text-blue-400" role="alert">
                    <svg class="flex-shrink-0 inline w-4 h-4 mr-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                ...
                <div class="relative sm:rounded-lg">
                    <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                      <div class="w-full md:w-1/2">
                        <form  action="{{route('admin.schedule.seats.search', ['schedule' => $schedules->id])}}" method="GET" class="flex items-center">
                          ...
                          <div class="relative w-full">
                            ...
                            <input type="text" id="simple-search" name="query" class="block w-full p-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-teal-500 focus:border-teal-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-teal-500 dark:focus:border-teal-500" placeholder="Search for seat">
                          </div>
                        </form>
                      </div>
                      <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                        <a href="{{route('admin.schedule.seats.add', ['schedule' => $schedules->id])}}" class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-teal-700 hover:bg-teal-800 focus:ring-4 focus:ring-teal-300 dark:bg-teal-600 dark:hover:bg-teal-700 focus:outline-none dark:focus:ring-teal-800">
                        ...
                        <div class="flex items-center w-full space-x-3 md:w-auto">
                          ...
                          <div id="actionsDropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow-md w-44 dark:bg-gray-700 dark:divide-gray-600">
                            <div class="py-1">
                                <button type="button" id="deleteButton" data-modal-target="deleteAll{{$schedules->id}}" data-modal-toggle="deleteAll{{$schedules->id}}" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
            ...
            <div class="block overflow-x-auto shadow-md bg-white dark:bg-gray-800">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400 rounded-lg">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td class="text-center px-6 py-3">
                                @if ($seat->seat_status == "available")
                                    <span class="text-green-600 font-bold capitalize">{{ $seat->seat_status }}</span>
                                @else
                                    <span class="text-red-600 font-bold capitalize">{{ $seat->seat_status }}</span>
                                @endif
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex items-center space-x-4 justify-center">
                                  <button type="button" data-modal-target="edit-seat{{$seat->id}}" data-modal-toggle="edit-seat{{$seat->id}}" class="py-2 px-3 flex items-center text-sm font-medium text-center text-white bg-teal-700 rounded-lg hover:bg-teal-800 focus:ring-4 focus:outline-none focus:ring-teal-300 dark:bg-teal-600 dark:hover:bg-teal-700 dark:focus:ring-teal-800">
                                  ...
                                  <button type="button" data-modal-target="seat-delete{{$seat->id}}" data-modal-toggle="seat-delete{{$seat->id}}" class="flex items-center text-red-700 hover:text-white border border-red-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-2 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">
                                      <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 -ml-0.5" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
                        ...
                        @include('admin.schedules.seats.crud.delete-seat')

                        @include('admin.schedules.seats.crud.edit-seat')
    ...
    @include('admin.schedules.seats.crud.deleteall-seat')
