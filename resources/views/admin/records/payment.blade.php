            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-t-lg">
                ...
                <div class="flex flex-col items-center justify-start p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                    <div class="w-full md:w-1/2">
                        <form action="{{route('admin.payment.search')}}" method="GET" class="flex items-center">
                    ...
                    <tbody>
                        ...
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            <td class="w-4 p-4">
                                <div class="flex items-center">
                                    <div class="flex items-center">
                                        <div class="text-base font-semibold">{{ $payments->firstItem() + $loop->index }}</div>
                                    </div>
                                </div>
                            </td>
            ...
            <div class="relative overflow-hidden bg-white rounded-b-lg dark:bg-gray-800">
                <nav class="flex flex-col items-start justify-between p-4 space-y-3 md:flex-row md:items-center md:space-y-0" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500 dark:text-gray-400">
                        Showing <span class="font-semibold text-gray-900 dark:text-white">{{ $payments->firstItem() }}-{{ $payments->lastItem() }}</span> of <span class="font-semibold text-gray-900 dark:text-white">{{ $payments->total() }}</span>
                    </span>
                    <ul class="inline-flex items-stretch -space-x-px">
                        ...
                        @foreach (range($startPage, $endPage) as $page)
                            <li>
                                <a href="{{ $payments->url($page) . (empty(request()->input('query')) ? '' : '&query=' . request()->input('query')) }}"
